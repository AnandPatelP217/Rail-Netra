...
def save_video_to_firebase_storage(fileName):
    ...
    blob = bucket.blob('work_monitoring_videos/' + fileName)
...
def save_to_firestore(platform, cctv_url, curr_time, curr_date, label, accuracy, video_url):
    doc_ref = db.collection(u'work_monitoring_videos').document()
    doc_ref.set({
        u'cctv_url': cctv_url,
        u'time': curr_time,
        u'date': curr_date,
...
def predict_on_live_cctv(CCTV_URL, platform):
    ...
    model = YOLO("newworkmonitoting.pt")  # work model

    ClassNames = ['emptychair', 'moving person', 'occupied chair', 'sitting person']
    ...
    alert_sent = False
    alert_threshold = 3 # The number of people to trigger an alert
    people_count = 0
    ...
    while True:
        ...
        for r in results:
            ...
            for box in boxes:
                ...
                if currentClass == 'emptychair':
                    myColor = (0, 255, 0)
                    people_count += 1  # Increment the people count for each detection
                    # Draw bounding box
                    cv2.rectangle(img, (x1, y1), (x2, y2), myColor, 1)
                    # Display class name
                    cv2.putText(img, currentClass, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, myColor, 1)
        cv2.putText(img, f"Workers missing from Seat: {people_count}", (30, 320), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                    myColor, 1)
        ...
        if time_difference >= image_capture_interval and people_count >= alert_threshold:
